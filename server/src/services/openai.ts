...
export async function analyzeText(content: string) {
  if (!client) {
    ...
    return {
      ...
      classification: content.toLowerCase().includes("agreement")
        ? "contract"
  ...
  const sys =
    "You are a legal document analysis assistant. Extract entities, summarize, classify (contract, motion, judgment, memo), and estimate risk low/medium/high.";
...
${content}`,
  ...
  return {
    ...
    entities: [],
    classification: text.toLowerCase().includes("contract")
      ? "contract"
